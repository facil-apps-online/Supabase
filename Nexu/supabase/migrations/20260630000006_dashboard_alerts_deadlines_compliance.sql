-- =============================================
-- Dashboard extensions: alerts, upcoming_deadlines, compliance
-- =============================================
-- Extends public.get_dashboard_stats with three new blocks so the
-- dashboard can render AlertList, UpcomingDeadlines and ComplianceChart
-- from a single round-trip:
--
--   alerts              : list of up to 8 derived alerts with counts
--   upcoming_deadlines  : list of up to 10 future events ordered by days_left
--   compliance          : per-module scheduled vs completed + overall
--
-- All data is scoped to the tenant resolved from the caller's JWT
-- (or to p_tenant_id when explicitly passed by a super admin).
-- Uses SECURITY DEFINER (following codebase pattern) so RLS policies
-- on intermediate tables (e.g. event_participants -> events) do not
-- cause infinite recursion during JOINs. Tenant isolation is enforced
-- manually via v_effective_tenant throughout the function body.
        ...
        (
            ...
            FROM (
                ...
                FROM (
                    ...
                    -- Courses: expiry_date in the future
                    SELECT jsonb_build_object(
                        'id',       'course-' || c.id,
                    ...
                    -- Committees: end_date in the future
                    SELECT jsonb_build_object(
                        'id',       'committee-' || c.id,
                    ...
                    -- Dotacion: delivery_date in the future
                    SELECT jsonb_build_object(
                        'id',       'dotacion-' || d.id,
