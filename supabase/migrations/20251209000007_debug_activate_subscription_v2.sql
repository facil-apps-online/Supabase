-- DEBUGGING version of activate_subscription to log the IF condition result.
...
DECLARE
    ...
    v_condition_check BOOLEAN; -- Variable to store the result of the condition
BEGIN
    ...
    -- Evaluate the condition and log the result explicitly
    v_condition_check := (v_active_subscription IS NOT NULL AND NOW() <= (v_active_subscription.end_date + '5 days'::interval));
    INSERT INTO public.debug_logs (message, details) VALUES ('[ACTIVATE_SUB] IF Condition Result', jsonb_build_object(
        'v_active_subscription_is_not_null', v_active_subscription IS NOT NULL,
        'old_end_date', v_active_subscription.end_date,
        'old_end_date_plus_grace', v_active_subscription.end_date + '5 days'::interval,
        'now_value', NOW(),
        'final_condition_evaluation', v_condition_check
    ));

    IF v_condition_check THEN -- Use the variable with the condition result
