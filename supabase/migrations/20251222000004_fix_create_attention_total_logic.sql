...
-- Reemplazamos la función con la lógica definitiva y correcta.
...
DECLARE
    ...
    v_subtotal_check numeric := 0; -- Usado para la verificación de seguridad
BEGIN
    -- Calcular subtotal de items conocidos por el backend para verificación
    IF jsonb_array_length(p_services) > 0 THEN
        FOR v_service IN SELECT * FROM jsonb_array_elements(p_services) LOOP
            v_subtotal_check := v_subtotal_check + COALESCE((v_service->>'price')::numeric, 0);
        END LOOP;
    END IF;
    IF jsonb_array_length(p_products) > 0 THEN
        FOR v_product IN SELECT * FROM jsonb_array_elements(p_products) LOOP
            v_subtotal_check := v_subtotal_check + (COALESCE((v_product->>'unit_price')::numeric, 0) * COALESCE((v_product->>'quantity')::integer, 1));
        END LOOP;
    END IF;
    IF jsonb_array_length(p_combos) > 0 THEN
        FOR v_combo IN SELECT * FROM jsonb_array_elements(p_combos) LOOP
            v_subtotal_check := v_subtotal_check + COALESCE((v_combo->>'price')::numeric, 0);
        END LOOP;
    END IF;

    -- Verificación de seguridad: el total del frontend no puede ser menor que la suma de los items que el backend conoce.
    IF p_total_amount < v_subtotal_check THEN
        RAISE EXCEPTION 'Mismatched total amount. Frontend total (%) is less than backend-calculated subtotal (%).', p_total_amount, v_subtotal_check;
    END IF;

    -- Insertar la atención usando el total validado del frontend
    ...
    -- Insertar servicios (usando el precio del payload, que será 0 para items de tratamiento)
    IF jsonb_array_length(p_services) > 0 THEN
    ...
    -- Insertar productos (usando el precio del payload)
    IF jsonb_array_length(p_products) > 0 THEN
    ...
    -- Notificaciones (se mantienen igual)
    BEGIN
