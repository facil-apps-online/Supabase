-- Migration to fix the get_public_subscription_plans RPC function.
-- Version: 20251104000010
...
BEGIN
    ...
    -- This CTE finds the relevant plan_country_configuration_id for each plan and the given country
    plan_configs AS (
    ...
    current_tariffs AS (
        SELECT DISTINCT ON (pt.subscription_plan_id)
            ...
            -- Corrected subquery for extra_branch_price
            (SELECT pal.overage_unit_price 
             FROM public.plan_asset_limits pal 
             WHERE pal.plan_country_config_id = (SELECT pcc_id FROM plan_configs pc WHERE pc.plan_id = pt.subscription_plan_id) 
               AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'suc_glam') LIMIT 1) AS extra_branch_price,
            -- Corrected subquery for included_einvoices
            (SELECT pal.value::INT 
             FROM public.plan_asset_limits pal 
             WHERE pal.plan_country_config_id = (SELECT pcc_id FROM plan_configs pc WHERE pc.plan_id = pt.subscription_plan_id) 
               AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'fe_glam') LIMIT 1) AS included_einvoices,
            -- Corrected subquery for extra_einvoice_price
            (SELECT pal.overage_unit_price 
             FROM public.plan_asset_limits pal 
             WHERE pal.plan_country_config_id = (SELECT pcc_id FROM plan_configs pc WHERE pc.plan_id = pt.subscription_plan_id) 
               AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'fe_glam') LIMIT 1) AS extra_einvoice_price,
            -- Corrected subquery for bonus (column does not exist, returning 0)
    ...
    SELECT
        ...
        pc.features AS plan_features, -- FIXED: Select features from plan_configs CTE
    ...
    JOIN plan_configs pc ON sp.id = pc.plan_id -- FIXED: Join with plan_configs to get features
