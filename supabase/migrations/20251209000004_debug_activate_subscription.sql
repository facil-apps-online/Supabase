-- DEBUGGING version of activate_subscription to log execution flow.
...
BEGIN
    RAISE NOTICE '[ACTIVATE_SUB] Started for tenant %', p_tenant_id;
    ...
    RAISE NOTICE '[ACTIVATE_SUB] Found plan_id: %', v_plan_id;
    ...
    RAISE NOTICE '[ACTIVATE_SUB] Searching for active subscription...';
    ...
    IF v_active_subscription IS NULL THEN
        RAISE NOTICE '[ACTIVATE_SUB] No active subscription found.';
    ELSE
        RAISE NOTICE '[ACTIVATE_SUB] Found active subscription with ID % and end_date: %', v_active_subscription.id, v_active_subscription.end_date;
    END IF;

    -- 6. Deactivate all current subscriptions for the tenant
    RAISE NOTICE '[ACTIVATE_SUB] Deactivating old subscriptions...';
    UPDATE public.tenant_subscriptions
    SET is_active = false
    WHERE tenant_id = p_tenant_id AND is_active = true;

    -- 7. Calculate new dates based on the (now deactivated) previously active subscription
    RAISE NOTICE '[ACTIVATE_SUB] Calculating new dates. Current time (NOW) is: %', NOW();
    IF v_active_subscription IS NOT NULL AND NOW() <= (v_active_subscription.end_date + '5 days'::interval) THEN
        RAISE NOTICE '[ACTIVATE_SUB] Stacking subscription. Using old end_date: %', v_active_subscription.end_date;
    ...
    ELSE
        RAISE NOTICE '[ACTIVATE_SUB] Not stacking. Starting new subscription from today.';
        v_new_start_date := NOW();
        v_new_end_date := NOW() + (v_billing_frequency_months || ' months')::interval;
    END IF;
    RAISE NOTICE '[ACTIVATE_SUB] New subscription will start on: % and end on: %', v_new_start_date, v_new_end_date;

    -- 8. Insert the new subscription record
    RAISE NOTICE '[ACTIVATE_SUB] Inserting new active subscription...';
    ...
    RAISE NOTICE '[ACTIVATE_SUB] Finished successfully.';
END;
$$;
