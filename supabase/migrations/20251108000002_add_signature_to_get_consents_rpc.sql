-- Adds the signature_file_id to the get_signed_consents_for_attention function
...
RETURNS TABLE(
    ...
    signature_file_id text -- Added column
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ...
        cs.google_drive_file_id AS signature_file_id -- Added column
    FROM
        public.signed_consents sc
    JOIN
        public.informed_consent_templates ct ON sc.template_id = ct.id
    LEFT JOIN
        public.consent_signatures cs ON sc.id = cs.signed_consent_id -- Added join
    WHERE
