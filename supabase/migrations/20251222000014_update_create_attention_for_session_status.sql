...
BEGIN
    ...
    -- 4. Actualizar sesiones de tratamiento
    UPDATE public.client_treatment_sessions
    SET
        status = 'Cita Asignada',
        attention_id = v_attention_id
    WHERE id IN (
        SELECT (jsonb_array_elements(p_services)->>'client_treatment_session_id')::uuid
        WHERE (jsonb_array_elements(p_services)->>'client_treatment_session_id') IS NOT NULL
        UNION
        SELECT (jsonb_array_elements(p_products)->>'client_treatment_session_id')::uuid
        WHERE (jsonb_array_elements(p_products)->>'client_treatment_session_id') IS NOT NULL
        UNION
        SELECT (jsonb_array_elements(p_payments)->>'client_treatment_session_id')::uuid
        WHERE (jsonb_array_elements(p_payments)->>'client_treatment_session_id') IS NOT NULL
    );
