-- Migration to fix ambiguous column reference in public plans RPC.
-- Version: 20251104000018
...
BEGIN
    ...
    SELECT
        ...
        (CASE WHEN ct.base_currency_code = cr.ccode THEN csa.extra_einvoice_price ELSE floor( (csa.extra_einvoice_price::numeric / (SELECT rate FROM rates_to_usd WHERE target_currency_code = ct.base_currency_code LIMIT 1)) * cr.usd_to_target_rate::numeric ) + 0.99 END)::numeric AS extra_einvoice_price,
