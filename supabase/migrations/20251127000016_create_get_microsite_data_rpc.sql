...
DECLARE
    ...
    v_branches json;
BEGIN
    -- 1. Find the tenant based on country ISO code and slug
    SELECT t.id, t.name, t.logo_url, t.slug INTO v_tenant
    FROM public.tenants t
    JOIN public.countries c ON t.country_id = c.id
    WHERE c.iso_code = p_country_iso_code AND t.slug = p_slug;
...
COMMENT ON FUNCTION public.get_microsite_data(text, text) IS 'Fetches all public data for a tenant''s microsite based on country and slug.';
