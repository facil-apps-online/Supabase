DROP FUNCTION IF EXISTS public.check_user_availability(uuid, text, date, time without time zone, integer, uuid, uuid, uuid, uuid);
...
RETURNS TABLE(user_id uuid, commission_rate numeric, first_name text, last_name text, is_active boolean, role_name text, is_favorite boolean)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    WITH RankedAssignments AS (
        SELECT
            ...
            (cp.user_id IS NOT NULL) as is_favorite,
