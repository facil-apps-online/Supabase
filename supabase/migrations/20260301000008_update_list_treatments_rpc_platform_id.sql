-- Migration: update_list_treatments_rpc_platform_id
-- Created at: 2026-03-01 00:00:08

DROP FUNCTION IF EXISTS public.list_treatments(uuid, text, boolean, uuid);
CREATE OR REPLACE FUNCTION public.list_treatments(p_tenant_id uuid, p_platform_id uuid, p_type text, p_show_inactive boolean DEFAULT false, p_category_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, name text, description text, type text, upfront_price numeric, financed_price numeric, is_active boolean, session_count bigint, cover_image_url text, categories jsonb, treatment_images jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
