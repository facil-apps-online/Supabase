...
CREATE OR REPLACE FUNCTION public.get_attentions_with_details(p_tenant_id uuid, p_branch_id uuid, p_user_id uuid, p_status_filter text, p_start_date date, p_end_date date)
 RETURNS TABLE(id uuid, created_at timestamp with time zone, tenant_id uuid, branch_id uuid, client_id uuid, attention_datetime timestamp with time zone, status text, notes text, informed_consent_id uuid, survey_token uuid, clients json, attention_services json, attention_products json, attention_combos json, attention_payments json)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
  ...
  attentions_filtered AS (
    SELECT
      a.id, a.created_at, a.tenant_id, a.branch_id, a.client_id, a.attention_datetime, a.status, a.notes, sc.id as informed_consent_id, ss.survey_token
  ...
  SELECT
    af.id, af.created_at, af.tenant_id, af.branch_id, af.client_id, af.attention_datetime, af.status, af.notes, af.informed_consent_id, af.survey_token,
