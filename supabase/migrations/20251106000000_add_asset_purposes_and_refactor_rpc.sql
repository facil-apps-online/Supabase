-- Migration to create a central asset_purposes table and refactor the public plans RPC.
-- Version: 20251106000000

BEGIN;

-- Step 1: Create the new asset_purposes table
CREATE TABLE public.asset_purposes (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
    purpose_key text NOT NULL,
    description text,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT asset_purposes_pkey PRIMARY KEY (id),
    CONSTRAINT asset_purposes_purpose_key_unique UNIQUE (purpose_key)
);
...
-- Step 2: Populate the asset_purposes table with known purposes
INSERT INTO public.asset_purposes (purpose_key, description)
VALUES
    ('extra_branch', 'Allows a tenant to operate an additional branch.'),
    ('e_invoice', 'Allows a tenant to issue an electronic invoice.'),
    ('storage', 'Provides storage space for tenant data.');

-- Step 3: Add the foreign key column to plan_assets
ALTER TABLE public.plan_assets
ADD COLUMN asset_purpose_id UUID,
ADD CONSTRAINT fk_asset_purpose FOREIGN KEY (asset_purpose_id) REFERENCES public.asset_purposes(id);

-- Step 4: Update existing assets to link them to their purpose
UPDATE public.plan_assets
SET asset_purpose_id = (SELECT id FROM public.asset_purposes WHERE purpose_key = 'extra_branch')
WHERE asset_key LIKE 'suc_%';

UPDATE public.plan_assets
SET asset_purpose_id = (SELECT id FROM public.asset_purposes WHERE purpose_key = 'e_invoice')
WHERE asset_key LIKE 'fe_%';

UPDATE public.plan_assets
SET asset_purpose_id = (SELECT id FROM public.asset_purposes WHERE purpose_key = 'storage')
WHERE asset_key LIKE 'spc_%';

-- Step 5: Make the new column NOT NULL after populating it
ALTER TABLE public.plan_assets
ALTER COLUMN asset_purpose_id SET NOT NULL;

-- Step 6: Refactor the get_public_subscription_plans function
