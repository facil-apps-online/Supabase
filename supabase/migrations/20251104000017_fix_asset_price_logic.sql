-- Migration to finally fix the asset price logic in the public plans RPC.
-- Version: 20251104000017
...
BEGIN
    ...
    -- This CTE gets the versioned base price for each plan (in COP)
    current_tariffs AS (
    ...
    -- This CTE gets the country-specific asset prices and limits
    country_specific_assets AS (
        SELECT
            pcc.plan_id,
            (SELECT pal.extra_unit_price FROM public.plan_asset_limits pal WHERE pal.plan_country_config_id = pcc.id AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'suc_glam')) AS extra_branch_price,
            (SELECT pal.value::INT FROM public.plan_asset_limits pal WHERE pal.plan_country_config_id = pcc.id AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'fe_glam')) AS included_einvoices,
            (SELECT pal.extra_unit_price FROM public.plan_asset_limits pal WHERE pal.plan_country_config_id = pcc.id AND pal.asset_id = (SELECT id FROM assets WHERE asset_key = 'fe_glam')) AS extra_einvoice_price
    ...
    SELECT
        ...
        -- Convert base plan price
        (CASE WHEN ct.base_currency_code = cr.ccode THEN ct.base_price ELSE floor( (ct.base_price::numeric / (SELECT rate FROM rates_to_usd WHERE target_currency_code = ct.base_currency_code LIMIT 1)) * cr.usd_to_target_rate::numeric ) + 0.99 END)::numeric AS calculated_price,
        -- Convert extra branch price (which is already country-specific)
        (CASE WHEN ct.base_currency_code = cr.ccode THEN COALESCE(csa.extra_branch_price, 0) ELSE floor( (COALESCE(csa.extra_branch_price, 0)::numeric / (SELECT rate FROM rates_to_usd WHERE target_currency_code = ct.base_currency_code LIMIT 1)) * cr.usd_to_target_rate::numeric ) + 0.99 END)::numeric AS calculated_extra_branch_price,
        ...
        csa.included_einvoices,
        -- Convert e-invoice price (which is already country-specific)
        (CASE WHEN ct.base_currency_code = cr.ccode THEN csa.extra_einvoice_price ELSE floor( (csa.extra_einvoice_price::numeric / (SELECT rate FROM rates_from_usd WHERE target_currency_code = ct.base_currency_code LIMIT 1)) * cr.usd_to_target_rate::numeric ) + 0.99 END)::numeric AS extra_einvoice_price,
    ...
    LEFT JOIN country_specific_assets csa ON sp.id = csa.plan_id
