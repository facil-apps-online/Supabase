...
DROP FUNCTION IF EXISTS public.create_full_attention(p_client_id uuid, p_attention_datetime timestamp with time zone, p_notes text, p_services jsonb, p_products jsonb, p_combos jsonb, p_tenant_id uuid, p_branch_id uuid);
...
-- Creación de la función definitiva con la firma y lógica correctas
CREATE OR REPLACE FUNCTION public.create_full_attention(
  ...
  p_products jsonb,
  p_combos jsonb,
  p_payments jsonb, -- NUEVO PARÁMETRO
  p_tenant_id uuid,
  p_branch_id uuid,
  p_total_amount numeric -- Se mantiene por compatibilidad pero se ignora
)
RETURNS uuid
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_product jsonb;
    v_combo jsonb;
    v_payment jsonb; -- NUEVA VARIABLE
    v_attention_combo_id uuid;
    v_combo_service_data jsonb;
    v_recalculated_total numeric := 0; -- El total se calcula 100% en el backend
BEGIN
    ...
    -- 2. Insertar la atención usando el total recalculado y seguro del backend
    INSERT INTO public.attentions (client_id, attention_datetime, notes, total_amount, tenant_id, branch_id, status)
    VALUES (p_client_id, p_attention_datetime, p_notes, v_recalculated_total, p_tenant_id, p_branch_id, 'Confirmada')
