-- Update get_tenant_plan_limits to accept p_tenant_id and p_platform_id
-- This matches the new signature sent from Edge Functions

DROP FUNCTION IF EXISTS public.get_tenant_plan_limits(uuid);
DROP FUNCTION IF EXISTS public.get_tenant_plan_limits(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_tenant_plan_limits(p_tenant_id uuid, p_platform_id uuid)
RETURNS TABLE (
  plan_name text,
  status text,
  is_trial boolean,
